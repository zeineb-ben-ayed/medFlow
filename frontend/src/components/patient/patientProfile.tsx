...
export const PatientProfile = ({
  ...
  const safeString = (v?: string | null, fallback = "—"): string =>
    v ?? fallback;
  const safeNumberOrString = (
    v?: string | number | null,
    fallback = "—"
  ): string | number => (v === null || v === undefined ? fallback : v);
  ...
  return (
    <div className="min-h-screen bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {stats.map((stat, index) => (
                      <div
                        key={index}
                        className="bg-white/60 backdrop-blur-sm rounded-lg p-3 border"
                      >
        ...
        >
          <TabsList className="p-1.5 border-2 shadow-sm w-full lg:w-auto bg-background">
            <TabsTrigger
              value="overview"
              className="flex-1 lg:flex-none data-[state=active]:bg-primary data-[state=active]:text-primary-foreground"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <Card className="border-2 shadow-sm hover:shadow-md transition-all lg:col-span-2">
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <InfoField
                      label="Date of Birth"
                      value={formatDateToLocale(patient?.dateNaissance)}
                      icon={Calendar}
                    />
                    <InfoField
                      label="Age"
                      value={`${calculateAge(patient?.dateNaissance)} years`}
                      icon={User}
                    />
                    <InfoField
                      label="Gender"
                      value={safeString(patient?.gender)}
                      icon={User}
                    />
                    <InfoField
                      label="Blood Type"
                      value={safeString(patient?.bloodType)}
                      icon={Droplet}
                    />
                    <InfoField
              ...
              <Card className="border-2 shadow-sm hover:shadow-md transition-all">
                ...
                <CardContent className="p-6 space-y-4">
                  <InfoField
                    label="Email"
                    value={safeString(patient?.email)}
                    icon={Mail}
                    fullWidth
                  />
                  <InfoField
                    label="Phone"
                    value={safeString(patient?.phoneNumber)}
                    icon={Phone}
                    fullWidth
                  />
                  <InfoField
                    label="Address"
                    value={safeString(patient?.address)}
                    icon={MapPin}
                    fullWidth
                  />
                </CardContent>
                    ...
                    <div className="bg-red-50 border-2 border-red-200 rounded-xl p-5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-foreground font-bold text-lg">
                          {safeString(patient?.emergencyName)}
                        </p>
                      </div>
                    </div>
                    <div className="bg-red-50 border-2 border-red-200 rounded-xl p-5">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-foreground font-bold text-lg">
                          {safeString(patient?.emergencyPhone)}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    {splitByComma(patient?.historiqueMedical).length === 0 ? (
                      ...
                      splitByComma(patient?.historiqueMedical).map(
                        (condition, index) => (
                          <div
                            key={index}
